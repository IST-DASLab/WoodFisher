...
FISHER_MINI=2400
MAX_MINI_BSZ=800
EXP_NAME="exp_str_prune_($MODEL)_gradmask_($TARGET)_epochs_($EPOCHS)_($PRUNER)_fit_($FITTABLE)_fisher_($FISHER_SUBSAMPLE)_($FISHER_MINI)_final_all_layers_best_disable_wdecay_after_prune"
...
--workers=24 --batch_size=256 --logging_level debug --gpus=0,1,2,3 \
