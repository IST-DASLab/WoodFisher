...
LAYER_INFO='all'
BSZ=180
EXP_NAME="exp_str_prune_($MODEL)_($TARGET)_${LAYER_INFO}_epochs_($EPOCHS)_($PRUNER)_($FISHER_SUBSAMPLE_SIZE)_($FISHER_MINI_BSZ)_($FITTABLE)_($BSZ)_flops-per-param_0.30_disable-wdecay-after-prune"
LOAD_FROM="../neural-compression/checkpoints/ResNet50-STR-Dense.pth"
CSV_DIR="./csv"
LOG_DIR="./logs"
mkdir -p ${CSV_DIR}
mkdir -p ${LOG_DIR}
...
--woodburry-joint-sparsify --offload-inv --offload-grads --flops-power 0.30 --flops-per-param --disable-wdecay-after-prune \
