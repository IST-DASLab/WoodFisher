...
LAYER_INFO='all'
BSZ=180
EXP_NAME="exp_str_prune_($MODEL)_($TARGET)_${LAYER_INFO}_epochs_($EPOCHS)_($PRUNER)_($FISHER_SUBSAMPLE_SIZE)_($FISHER_MINI_BSZ)_($FITTABLE)_($BSZ)_flops-target-335_per-param_0.30_disable-wdecay-after-prune_1"
...
--woodburry-joint-sparsify --offload-inv --offload-grads --flops-target 335 --flops-epsilon 0.1 --flops-power 0.30 --flops-per-param --disable-wdecay-after-prune \
